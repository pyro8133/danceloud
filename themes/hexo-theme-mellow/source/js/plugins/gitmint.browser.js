...
function renderCounter(_ref4, instance) {
  var meta = _ref4.meta;
  var id = instance.id;
  var counterParent = [...document.querySelectorAll(".post-comments-counter")].forEach(item => {
      if (item.id === id) {
          var counter = item.querySelector(".post-comments-count.gitment-comments-count");
          if (counter &&
...
var Gitment = function () {
  ...
  function Gitment() {
    ...
    if (query.code) {
      ...
      var logging = !proxy_gateway ? _utils.http.post('https://service.lujingtao.com/gitment', { code: code, client_id: client_id, client_secret: client_secret }, '') : _utils.http.post('/login/oauth/access_token', 'code=' + code + '&client_id=' + client_id, proxy_gateway);
